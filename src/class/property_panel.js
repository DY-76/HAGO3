           ...
           $('.help_div').before($("<p id='Paneltip' style='position:relative; bottom:40px; width:300px'>").html("<span style='font-size:27px; position:relative; top:10px;'>도움말</span><br>__________________________"));
            switch (Number(getParameterByName('stage'))){
                case 1:
                    ...
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 빈집만 상습적으로 도둑질하는 도둑들이 빈집을 찾다가 학생네 집에 가족들이 모두 여행을 가고 학생 혼자만 있는걸 알게 되었어요. 도둑들이 학생네 집에 침투하지 못하도록 학생을 도와 도둑을 막아봅시다."));
                    $('.help_div').append($("<p id='Paneltip'>").html("▶도둑들이 성휘 집으로 못 들어 오도록 상자로 문을 막아봅시다."));
                ...
                case 2 :
                    ...
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 도둑들이 학생의 집으로 못 들어 오도록 상자로 문을 막아봅시다."));
                ...
                case 4 :
                    ...
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 도둑 한명이 계란을 맞고 아래로 떨어졌네요. 그것을 지켜보던 다른 도둑이 화가 나서 2층으로 올라오는데 남은 계란이 없네요. 학생은 미리 이것을 염두해두고 2층의 불을 다 끈 상태로 빛을 이용해 도둑을 막으려고 합니다. 빛을 이용해 도둑이 집안으로 들어오지 못하도록 친구들이 도와주세요 !"));
                ...
                case 5 :
                    ...
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 도둑을 필사적으로 막아보았지만, 결국에 도둑이 2층 집안으로 들어와버렸습니다. 학생은 2층 계단으로 향하는 문을 닫고 문 손잡이에 전류를 흐르게 하여 도둑들이 문을 못 열게 하려고 합니다. 도체를 이용해 문 손잡이에 전류를 흐르게 친구들이 도와주세요 !"));
                ...
                case 7 :
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 성취기준 : 정보 [9정03-01], [9정04-04] / 과학 [9과09-03], [12물리Ⅰ02-03]"));
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 친구들의 도움이 있어서 다행히 2층에서 시간을 벌었어요. 하지만 도둑은 포기하지 않고 학생을 끝까지 추격하려고 하네요. 학생은 미리 1층에 내려와 호스를 이용해 물 대포를 쏴서 도둑을 지하로 떨어뜨리려고 하려면 친구들이 도와줘야 해요 !"));
                ...
                case 8 :
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 성취기준 : 정보 [9정03-01], [9정04-04] / 과학 [9과09-03], [12물리Ⅰ02-03]"));
                    $('.help_div').append($("<p id='Paneltip'>").html("▶ 물 대포를 이용해 도둑이 지하실로 떨어진 틈을 타서 학생이 2층으로 도망가는데 성공을 했어요. 근데 생각보다 도둑이 정신을 빨리 차리고 1층으로 올라와버렸어요. 도둑이 계단을 이용해 2층으로 올라오는 동안 타이밍을 맞춰서 공을 떨어트려 도둑을 맞춰 봅시다 !"));
